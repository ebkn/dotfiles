...
3. **Primary language** — one of: `typescript`, `go`, `python`, `next` for Next.js, `ai-agent` for an AI agent CLI/worker app on Node.js, or `workers` for a Cloudflare Worker. `next`, `ai-agent`, and `workers` are TypeScript overlays with their own scaffolds; other framework answers (FastAPI, Gin, …) just select their language.
4. **Project type** — one of: `public-web` (public, indexable site), `internal-web` (internal tool / dashboard), `api` (backend/HTTP service, no browser UI), or `library`/`cli`. This gates the conditional steps: SEO scaffold (`public-web` only), security headers (any served web UI), and the health endpoint (`api` or any HTTP server). For `ai-agent` this answer is always `library`/`cli`; the overlay asks its own shape question (`cli` vs `worker`) instead. For `workers` the default is `api` (the fetch handler is the server; its reference ships the health endpoint).

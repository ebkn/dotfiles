...
| Swift | `xcodegen generate` | `xcodebuild test -project {AppName}.xcodeproj -scheme {AppName} -destination 'platform=macOS'` | `swift format lint --strict --recursive {AppName} {AppName}Tests` · `swiftlint --strict` | `xcodebuild build -project {AppName}.xcodeproj -scheme {AppName} -destination 'platform=macOS'` |

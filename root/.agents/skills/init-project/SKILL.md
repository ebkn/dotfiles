...
3. **Primary language** — one of: `typescript`, `go`, `python`, `next` for Next.js, `ai-agent` for an AI agent CLI/worker app on Node.js, `workers` for a Cloudflare Worker, or `swift` for a native macOS SwiftUI app. `next`, `ai-agent`, and `workers` are TypeScript overlays with their own scaffolds; other framework answers (FastAPI, Gin, …) just select their language.
4. **Project type** — one of: `public-web` (public, indexable site), `internal-web` (internal tool / dashboard), `api` (backend/HTTP service, no browser UI), or `library`/`cli`. This gates the conditional steps: SEO scaffold (`public-web` only), security headers (any served web UI), and the health endpoint (`api` or any HTTP server). For `ai-agent` this answer is always `library`/`cli`; the overlay asks its own shape question (`cli` vs `worker`) instead. For `workers` the default is `api` (the fetch handler is the server; its reference ships the health endpoint). For `swift` skip this question — a desktop app matches none of the types; every web-conditional step is skipped, and `references/swift.md` asks its own intake questions (app name, bundle ID prefix, minimum macOS version).
...
7. **Error tracking** — `sentry` or `none`. Selects the optional Sentry overlay (`references/sentry.md`), which wires the SDK for whichever language was chosen. Ask this **without** requiring a Sentry account: the scaffold reads the DSN from the environment and every SDK here no-ops when it is unset (verified per-SDK in that reference), so answering `sentry` costs a dependency and some config, not a signup. Two answers are decided for the user rather than asked: skip the question entirely for a **`library`** (a library must never call `Sentry.init` on its host's behalf — the reference explains why), and skip it for **Swift** unless the user brings it up, since a sandboxed macOS app needs an entitlement change to send anything.
...
| AI agent (Node.js CLI/worker) | `references/typescript.md`, then `references/ai-agent.md` |
| Cloudflare Workers | `references/typescript.md`, then `references/cloudflare-workers.md` |
| Go | `references/go.md` |
| Python | `references/python.md` |
| Swift (macOS app) | `references/swift.md` |
| All of the above | `references/supply-chain.md` for CI, Dependabot, and pinning |
| Error tracking, if intake question 7 answered `sentry` | `references/sentry.md` — read it at Step 9, **after** the language reference and CI, not instead of them |
...
| AI agent | `npm install` | `npm test` | `npm run lint` · `npm run typecheck` · `npm run knip` | — |
| Cloudflare Workers | `npm install` | `npm test` | `npm run lint` · `npm run typecheck` · `npm run knip` | — |
...
| Swift | `make generate` | `make test` | `make lint` (check) · `make format` (write) | `make build` · `make clean` |

Multiple commands in one cell go on separate lines inside the *same* fenced block of their README section — one fence per section, not per command. Swift is the one row whose commands are not the tool's own: it has no task runner, so `references/swift.md` scaffolds a `Makefile` that owns the long `xcodebuild` invocations, and README, CLAUDE.md, and CI all call the targets. Write the targets here, not the commands behind them. Where the Build cell is "—" the language genuinely has no build step (plain TypeScript sets `noEmit: true`, so `typecheck` is the only `tsc` run; the AI agent overlay runs `src/` directly via Node's TypeScript type stripping; Cloudflare Workers is bundled by `wrangler` at dev/deploy time; Python has no compile). Do not invent one: write "No build step" with the one-line reason under the Build heading instead of leaving it dangling or mislabeling `typecheck` as a build. Go commands that embed a resolved version (`govulncheck@{version}`, `deadcode@{version}`) are resolved in Steps 7–8 — backfill them into both files then rather than resolving out of order here.
...
**Expect to be asked before this one runs, and do not rewrite it to avoid the prompt.** Claude Code always prompts for a command containing process substitution — `<(…)` — because what it expands to cannot be read from the command string, so **no `allow` entry pre-approves this**, `Bash(diff *)` included. In a non-interactive run it is refused outright; compare the two blocks with `Read` and say in the report that the mechanical check could not run. Reaching for a substitute that happens to match an allow pattern is how a verification turns into something weaker that merely looks green.
...
- The **error-tracking bullet is written twice over**, so write the one that matches intake question 7. Leave it as-is for `none`. For `sentry`, Step 9 replaces it with what is actually left to do — the code is wired, the account is not — so do not leave a bullet claiming error tracking is unscaffolded next to a tree that has `Sentry.init` in it. `references/sentry.md` supplies the replacement wording.
...
**Secrets — always deny, all languages.** This scaffold is agent-first, and every stack here reaches for a `.env`-family file (Next.js is told outright to put real values in `.env.local`; Python uses `dotenv`, Go `godotenv`). Without a `deny` rule, a prompt-injection payload in any content Claude reads can have it `cat .env` and exfiltrate the secrets — an `allow` list doesn't stop that, because anything not allowed merely *prompts*, and an unattended or over-eager agent answers the prompt. `deny` is a no-prompt hard block that wins over both `ask` and `allow` ([precedence: deny → ask → allow](https://code.claude.com/docs/en/permissions)), and a `Read` deny also covers the file-reading Bash commands Claude Code recognizes (`cat`, `head`, `tail`, `sed`) — so it closes both the tool read and the shell read.
...
**This step cannot complete in a non-interactive run.** `.claude/settings.json` is special-cased by Claude Code: writing it always requires an approval prompt, and **an `allow` entry for `Write` does not cover it** — the restriction exists precisely because the file governs permissions, so a run that could write it unattended could widen its own. Under `claude -p` with prompts refused, the `Write` is denied and there is nothing to work around. Do not retry it through `Bash` (that is denied too, and would be a way around a deliberate boundary). **Say so in the final report instead**, naming `.claude/settings.json` and what it was going to contain, so the caller can create it themselves — a scaffold that reports success with this step silently missing is the failure to avoid.
...
Ignore `.env*` wholesale rather than only `.env*.local`. The Next.js reference has the user keep real values in `.env.local` (the convention devs arriving from other repos expect, and the one file Next.js skips when `NODE_ENV=test`), which a `*.local` glob would already catch — but wholesale `.env*` is what the pinned `create-next-app` template itself ignores, and it is strictly safer: a secret fat-fingered into `.env` (or `.env.production`) instead of `.env.local` is still never tracked. That defense-in-depth is the point — the ignore rule shouldn't depend on the user picking the exact filename we recommended. The `!.env.example` negation keeps the committed template visible.
...
Follow `references/sentry.md`. Skip this step entirely otherwise; nothing later depends on it.

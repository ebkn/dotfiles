...
The `vcs` block makes Biome honor `.gitignore`. **Do not omit it.** Biome 2.x does *not* read `.gitignore` by default, so without this it lints and formats generated build output the moment it exists — `.next/` and `out/` (Next.js), `dist/` (a library build). `useIgnoreFile: true` makes `.gitignore` the single source of truth for what tooling ignores, matching how the rest of this scaffold is organized. For that to hold, `.gitignore` must already exist the first time Biome runs — which is why SKILL.md Step 6 creates it before language setup and verification; see that step for the ordering rationale (build-before-lint, and why a late `.gitignore` pushes an agent toward a `biome.json` exclude that undoes this design) rather than repeating it here.

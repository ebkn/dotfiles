...
**Boundary of this skill: read and report only.** Make no file changes and no git operations (add / commit / push, etc.). **This is a rule you enforce yourself. Never infer permission from the environment.** Read/search/list files through the `Read`/`Grep`/`Glob` tools, not shell.

On Claude Code the grant does back the rule up: `allowed-tools` pre-authorizes only read-only `Bash` — read-only git (`git log`/`show`/`diff`/`status`/`blame`/`ls-files`), package audits/inventory (`npm/pnpm/yarn/bun audit`, `ls`, `outdated`), dead-code/unused-dependency analysis (`npx knip`), `lighthouse`, `curl` (for response inspection), `yq` (for reading workflow/Dependabot YAML), and `git ls-remote` (for confirming an action pin resolves to its tag's peeled commit). There, any other command — including anything that writes or mutates git — prompts for approval; a read-only inspector for another ecosystem (`go list`, `cargo tree`, `pip-audit`, etc.) prompting is expected, but a write/commit/push prompt means something is wrong — do not approve it.

**Other hosts ignore `allowed-tools` entirely and give this skill no tool-level fence at all.** Codex reads only `name` and `description` from the frontmatter, and its own prefix rules pre-approve `git add`, `git commit -m`, `git push`, and `gh pr create`/`edit` — so there the forbidden operations run silently, with no prompt. **Absence of a prompt is therefore not permission.** The read-only guarantee comes from this paragraph, not from the harness. `WebSearch`/`WebFetch` are for verifying advisories (CVEs, EOL dates), framework-version-specific behavior, and pulling the **current** official best-practice docs for the detected stack (each checklist file lists its own sources). Because Phase 2 fans the per-bucket passes out to sub-agents, note that **the read-only guarantee is weaker for those sub-agents than for the orchestrator:** whatever tool-level grant constrains *this* agent's shell access does not automatically constrain a sub-agent's. Keep them read-only by construction — delegate to a sub-agent type that cannot edit or write files — plus a verbatim read-only instruction in every prompt; that is enforcement at the prompt level, not the grant level. Applying any fix is a separate request.
...
### Every item carries its own verification — run it, don't read it

Each item in every checklist file names two things after the item text:

- **`verify:`** — how to establish the item's state. Some are a `path:line` search; many are a command to run (`curl -I`, `npm audit`, `npx knip`, `lighthouse`), and a few can only be answered by a human with console access.
- **`pass:`** — the concrete condition that counts as OK. Without it, "Timeouts on all outbound calls" has no stated bar and two passes of this skill will disagree about the same code.

**Where an item's `verify:` names a command that can be run here, running it is not optional.** Reading a config file and concluding that a header will be sent is an inference; a response with the header in it is a fact. The two disagree more often than they should, because between the config and the wire sit route matchers, framework defaults, middleware ordering, and the platform's own edge layer. A judgment that could have been settled by an available command and was not is a **Cannot verify**, not an OK — record it as such rather than upgrading an inference to a pass.

Say which of the two you did. A finding sourced from a run carries the command and its output; a finding sourced from reading carries `path:line`. The report format below marks this explicitly, because a reader cannot otherwise tell a measured result from a plausible one.

### Get something running, early

The single biggest determinant of how much this check can actually settle is whether there is a **reachable URL**. Establish this in Phase 1, before fanning out, and hand the answer to every bucket:

1. **A production or staging URL**, if the app is already deployed. Best case — headers, TLS, redirects, `robots.txt`, and real Core Web Vitals are all directly observable, and they are observable *as the platform actually serves them*, which is the only configuration that matters.
2. **A locally running instance** — a production build the requester starts (`npm run build && npm start`, `wrangler dev`, the project's own equivalent). This settles application-level behavior — routes, headers set by the app, health endpoints, error pages — but **not** anything the platform adds or overrides: CDN caching, edge redirects, WAF, TLS, HSTS in force.
3. **Neither.** Then say so in the Overview, and mark every runtime item Cannot-verify rather than substituting a source-read for it.

**Do not build or start the app yourself.** A production build writes into the working tree (`.next/`, `dist/`, `worker-configuration.d.ts`), and this skill's boundary is read-and-report — a gitignored artifact is still a file change, and "it's only generated output" is exactly the reasoning that erodes a boundary until it is gone. Ask the requester to start it and tell you the URL; that is one question, and it converts a large block of this checklist from inference to measurement. If they decline or it is impractical, take option 3 and say so — an honest Cannot-verify is worth more than a check bought by breaking the skill's own contract.

When running locally, note the gap explicitly in each affected finding: a header present on `localhost:3000` is evidence the app sets it, not evidence production sends it — a reverse proxy or CDN can strip or replace it. Never report a locally-observed header as a production fact.
...
- **A reachable URL** — production/staging, or a local production build, or neither (see *Get something running, early* above). Record which one, verbatim, in the stack profile: every bucket's runtime items depend on it, and a bucket that guesses wrong either skips checks it could have run or reports a localhost observation as production truth.
...
**Dispatch.** Using whatever concurrent sub-agent mechanism the running tool provides, launch all applicable buckets **at once** so they run in parallel — not one after another. Delegate to a **read-only sub-agent type — one that cannot edit or write files** — and select each sub-agent's model per the routing table below. Give every agent:

- the Phase-1 **stack profile** (so it does not re-detect the stack or diverge),
- the path to its checklist file (`checklists/<name>.md`) — or its contents, if the tool doesn't expose this skill's directory to sub-agents,
- the read-only contract, verbatim: **gather evidence with read-only commands only; never edit or write a file, and never run a mutating or version-control-writing command**,
- **the reachable URL from the stack profile, and the instruction to run every item whose `verify:` names a runnable command** — a bucket that only reads source produces inferences, and the orchestrator cannot tell those from measurements after the fact,
- the return contract: report each item as OK / Gap / N/A / Cannot-verify, say for each whether it was **measured** (command + its output) or **read** (`path:line`), include any `ref:` source URL, and **do not assign final P1/P2/P3** — the orchestrator classifies centrally in Phase 3 so priorities stay consistent across buckets.
...
**Model routing — match the model tier to the bucket's reasoning demand.** These are tiers, not product names: map each to the running provider's actual lineup — the strongest reasoning model, a balanced mid model, a fast lightweight model. **Upgrade** a bucket when the app makes it unusually subtle (a hand-rolled auth or payment layer, a bespoke caching scheme); **downgrade** when it's trivially thin.

| Tier | Buckets | Why |
|---|---|---|
| **Top (deep reasoning)** | `backend-api`, `security`, `payments`, `nextjs` | Subtle, high-stakes correctness — SSRF / cache-leak / transaction / idempotency, authz / IDOR / injection / CSP, money-path integrity, RSC-boundary / caching / rendering. A miss here is a P1. |
| **Mid (balanced)** | `frontend-web`, `frontend-a11y`, `seo`, `aio-llmo`, `legal`, `observability`, `delivery-infra`, `forms` | Real judgment but more pattern-based; several are WebSearch-driven (fast-moving SEO/AIO) or config-presence checks with moderate nuance. |
| **Fast (lightweight)** | `manual-setup` | Mostly presence-of-wiring and cannot-verify questions (verification tokens, DSN, external console steps) — little code reasoning. |

**Synthesis stays on the orchestrator** — this agent, running at full strength. Dedup and P1/P2/P3 classification (Phase 3) need the strongest reasoning and the full cross-bucket view; do **not** offload or downgrade them.

**Honest trade-off (accepted for parallelism).** Fan-out means the sub-agents' shell access is not scoped the way this orchestrator's is — read-only is enforced at the prompt level, with a read-only sub-agent type (one that cannot edit or write files) as the structural backstop. Keep the read-only instruction explicit in every sub-agent prompt.

**Fallback (no sub-agents, or no per-agent model control).** If the running tool can't spawn sub-agents or can't set their models individually, run the buckets **sequentially in this agent** instead: complete and record one checklist file before loading the next — never hold all buckets in focus at once — using the best available model throughout. This preserves the per-bucket focus (the point of the split) and keeps any grant-backed read-only enforcement fully in force, at the cost of wall-clock.
...
- **Confirm every candidate P1 before it is reported as one.** A P1 is an assertion that the app will break, leak, or lose data on launch, and the cost of being wrong is asymmetric in both directions: a false P1 spends someone's launch day on a non-issue and teaches them to discount this report, while a missed one is the whole reason the check exists. For each candidate, try to **refute** it — is the sink actually reachable from user input, is the route actually deployed, is the config actually the one in force, does the framework already handle it? Re-read the surrounding code rather than the matched line. Where the finding names something observable and a URL is reachable, settle it with a command instead of an argument. A candidate that survives refutation is a P1; one that cannot be confirmed drops to P2 with the uncertainty stated, and one that is refuted is dropped with no trace in the report.
...
- Runtime checks: <the URL exercised — production, staging, or a local build — or "none reachable: every runtime item is Cannot-verify">

Findings carry their evidence and how it was obtained. `evidence:` is either **measured** — the command run and the relevant line of its output — or **read** — the `path:line` in this repo. Mark which; a reader must be able to tell a response that was observed from a config that was interpreted, and a locally-observed result must say so rather than being reported as production behavior. `ref:` is the external best-practice source URL the judgment rests on (include `ref:` whenever the finding leans on a live doc or a fast-moving-area lookup; omit it for a plain in-repo defect). If the run was degraded, mark any finding whose `ref:` could not be fetched as **unverified against current docs**.

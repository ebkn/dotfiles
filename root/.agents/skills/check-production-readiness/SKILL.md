...
On Claude Code the grant does back the rule up: `allowed-tools` pre-authorizes only read-only `Bash` — read-only git (`git log`/`show`/`diff`/`status`/`blame`/`ls-files`), package audits/inventory (`npm/pnpm/yarn/bun audit`, `ls`, `outdated`), dead-code/unused-dependency analysis (`npx knip`), `lighthouse`, and `curl` (for response-header inspection). There, any other command — including anything that writes or mutates git — prompts for approval; a read-only inspector for another ecosystem (`go list`, `cargo tree`, `pip-audit`, etc.) prompting is expected, but a write/commit/push prompt means something is wrong — do not approve it.

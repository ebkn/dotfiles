...
Check whether a web application is ready to go public. This skill is an **orchestrator**: it detects the stack, selects which checklist buckets apply, then runs one **focused pass per bucket** — because a single 140-item sweep in one context gets skimmed, not honored. The detailed items live in `checklists/*.md`, one file per bucket; this file holds the orchestration, the classification rules, and the report format.
...
### Phase 2: Run one focused pass per applicable bucket

For each selected bucket, run a pass that:

1. **Reads only that bucket's checklist file** (`checklists/<name>.md`) — so ~15 items are in focus, not 140.
2. Fetches that file's best-practice sources (the live doc wins over the file) and folds any new/changed guidance in as extra checks. Fast-moving areas (SEO/AIO/LLMO, platform features, advisories) especially — prefer the latest authoritative source over the file when they differ.
3. Audits the repo against every item, grounding each in `path:line` and landing it in OK / Gap / N/A / Cannot-verify.
4. Returns its findings — do **not** assign final P1/P2/P3 inside the pass; that classification is centralized in Phase 3 so it stays consistent across buckets.

**Default execution — sequential, by this (main) agent.** Complete one bucket fully and record its findings **before** loading the next file. Do not hold all buckets in focus at once. This keeps the grant-backed read-only enforcement fully in force and is what fixes the skimming.

**Optional execution — parallel fan-out (Claude Code), for speed.** You *may* dispatch the bucket passes concurrently as read-only sub-passes. On Claude Code use the **`Explore`** agent type (it cannot `Edit`/`Write`). Give each sub-pass: the Phase-1 **stack profile**, the path to its checklist file (or its contents if the tool doesn't expose this skill's directory to subagents), and an explicit instruction to **gather evidence with read-only commands only — never mutate the repo or git**. Note the trade-off honestly: fan-out relaxes the grant-backed read-only guarantee to a prompt-level one (a subagent's `Bash` isn't scoped the way this agent's is), so keep that instruction explicit and prefer the sequential default when read-only certainty matters more than wall-clock.

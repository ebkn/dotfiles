...
このスキルの目的は、**レビュワーが対象PRを「理解」すること**を支援することにある。まず理解の起点を示し、そこから**対話を通じて理解を深める**。レビュー内容としてまとめるのは、理解が固まった後、レビュワーが求めたときだけ行う。**レビューの送信は必ずレビュワー自身が行い、このスキルは投稿しない。**

**このスキルの境界: 読み取りと対話のみ。** GitHub への書き込み（`gh pr review` / `gh pr comment` / `gh pr edit` / `gh pr create` / `gh api` の POST・PATCH など）と、リポジトリへの書き込み（`git add` / `git commit` / `git push`）は一切行わない。レビューの送信はレビュワー自身の操作であり、代行しない。まとめたレビュー案はチャット上に出力するだけに留める。

**この境界は自分で守るルールであり、実行環境に依存しない。** frontmatter の `allowed-tools` を尊重しないホストがある（Codex は `name` と `description` しか読まない）。そうしたホストでは `gh pr create` / `gh pr edit` や `git commit` / `git push` が事前承認済みで、確認プロンプトなしにそのまま実行される。**プロンプトが出ないことは許可を意味しない。**

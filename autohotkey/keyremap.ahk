...
; ===================================================================
; Window Snapping (Magnet-like)
;
; Win+Ctrl+H : left half of the current monitor
; Win+Ctrl+L : right half of the current monitor
; Win+Ctrl+M : maximize
;
; The current monitor is chosen by the active window's center point so
; multi-monitor setups behave as expected.  MonitorGetWorkArea excludes
; the taskbar.
; ===================================================================

#^h::SnapActiveWindow("left")
#^l::SnapActiveWindow("right")
#^m::WinMaximize("A")

SnapActiveWindow(side) {
    if !WinExist("A")
        return
    ; WinMove on a maximized window leaves it stuck at full size; restore first.
    if (WinGetMinMax("A") != 0)
        WinRestore("A")
    ...
    halfW := (right - left) // 2
    x := (side = "left") ? left : left + halfW
    WinMove(x, top, halfW, bottom - top, "A")
}
